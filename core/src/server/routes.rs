...
pub(crate) async fn synthesize_project_briefing(
    ...
    let now = Utc::now();
    // Strict per-project: facts must carry THIS project's tag. include_global was
    // pulling untagged/global facts into every project's briefing (e.g. localmem's
    // own notes leaked into a StudentHousing brief). Understandings are already
    // strict (gather_understandings), so this aligns facts with them.
    let scope = project_scope((!project.is_empty()).then_some(project), false);
    let mut facts = {
